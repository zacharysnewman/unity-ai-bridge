...
**Multiple components of the same type** are supported. The Nth entry for a given `type` maps to the Nth component of that type on the GameObject (by Unity's component order). On load, `SceneIO` uses `GetComponents<T>()` and matches by index.
